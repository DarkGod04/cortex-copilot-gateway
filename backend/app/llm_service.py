...
def ask_copilot(user_query: str, tenant_id: str, data_context: dict) -> str:
    ...
    system_prompt = (
        ...
        f"--- DATA CONTEXT ---\n"
        f"Estimated Bill: ₹{data_context.get('estimated_bill_inr', 'N/A')}\n"
        f"Energy Charges: ₹{data_context.get('energy_charge_inr', 'N/A')} (Consumption: {data_context.get('total_kvah', 'N/A')} kVAh)\n"
        f"Demand Charges: ₹{data_context.get('demand_charge_inr', 'N/A')}\n"
        f"Peak Demand: {data_context.get('max_demand_kva', 'N/A')} kVA\n"
        f"Power Factor: {data_context.get('avg_power_factor', 'N/A')}\n"
        f"System Anomalies: {data_context.get('system_anomalies', 'None detected.')}\n" # <--- ADDED ANOMALIES
        f"--------------------\n\n"
        ...
        f"4. ANOMALIES: If 'System Anomalies' is not 'None detected.', you MUST append a final bolded bullet point starting with 'CRITICAL ALERT:' detailing the exact anomaly." # <--- ADDED RULE 4
    )
    ...
    response = ollama.chat(
        ...
        messages=[
            ...
            # --- ADDED: Training the LLM to format the anomaly correctly ---
            {'role': 'user', 'content': 'give me a summary of my metrics'},
            {'role': 'assistant', 'content': f"* **Peak Demand:** {data_context.get('max_demand_kva')} kVA\n* **Power Factor:** {data_context.get('avg_power_factor')}\n* **CRITICAL ALERT:** {data_context.get('system_anomalies', 'None detected.')}"},
            ...
            {'role': 'user', 'content': 'Write a fictional story about a factory.'}, # <--- Added the storytelling trap back in
            {'role': 'assistant', 'content': 'Data unavailable under current tenant configuration.'},
